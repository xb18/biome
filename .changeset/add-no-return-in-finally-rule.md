...
Added the nursery rule [`noReturnInFinally`](https://biomejs.dev/linter/rules/no-return-in-finally/). This rule disallows return statements in `finally()` promise callbacks, including inside nested blocks and conditional branches. Returns in nested functions are allowed.
